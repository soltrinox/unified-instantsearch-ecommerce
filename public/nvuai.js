...
(function (root, factory) {
  ...
  if (typeof define === 'function' && define.amd) {
    // AMD + global
    define([], function () {
      return root.nvuai = factory(root);
    });
  ...
  nvuai = {
    /**
     * Add commands that nvuai will respond to. Similar in syntax to init(), but doesn't remove existing commands.
     *
     * #### Examples:
     * ````javascript
     ...
     * nvuai.addCommands(commands2);
     * // nvuai will now listen for all three commands
     * ````
     *
     * @param {Object} commands - Commands that nvuai should listen for
     * @method addCommands
     ...
     *
     * - `autoRestart`  (boolean) Should nvuai restart itself if it is closed indirectly, because of silence or window conflicts?
     * - `continuous`   (boolean) Allow forcing continuous mode on or off. nvuai is pretty smart about this, so only set this if you know what you're doing.
     * - `paused`       (boolean) Start nvuai in paused mode.
     *
     * #### Examples:
     * ````javascript
     * // Start listening, don't restart automatically
     * nvuai.start({ autoRestart: false });
     * // Start listening, don't restart automatically, stop recognition after first phrase recognized
     * nvuai.start({ autoRestart: false, continuous: false });
     * ````
     ...
     *
     * Alternatively, to only temporarily pause nvuai responding to commands without stopping the SpeechRecognition engine or closing the mic, use pause() instead.
     * @see [pause()](#pause)
     *
     * @method abort
     */
    ...
    /**
     * Pause listening. nvuai will stop responding to commands (until the resume or start methods are called), without turning off the browser's SpeechRecognition engine or the mic.
     *
     ...
     * @method setLanguage
     * @see [Languages](https://github.com/TalAter/nvuai/blob/master/docs/FAQ.md#what-languages-are-supported)
     */
     ...
     * nvuai.removeCommands();
     *
     * // Add some commands
     * nvuai.addCommands(commands);
     *
     * // Don't respond to hello
     * nvuai.removeCommands('hello');
     *
     * // Don't respond to howdy or hi
     * nvuai.removeCommands(['howdy', 'hi']);
     * ````
     ...
     *
     *     This will fire once per Speech Recognition starting. See https://is.gd/nvuai_sound_start.
     *
     ...
     *
     * * `resultMatch` - Fired when nvuai was able to match between what the user said and a registered command.
     *
     ...
     * #### Examples:
     * ````javascript
     * nvuai.addCallback('error', function() {
     *   $('.myErrorText').text('There was an error!');
     * });
     *
     * nvuai.addCallback('resultMatch', function(userSaid, commandText, phrases) {
     ...
     * nvuai.addCallback('errorNetwork', notConnected, this);
     * ````
     ...
     * #### Examples:
     * ````javascript
     * nvuai.addCallback('start', myFunction1);
     * nvuai.addCallback('start', myFunction2);
     * nvuai.addCallback('end', myFunction1);
     * nvuai.addCallback('end', myFunction2);
     *
     * // Remove all callbacks from all events:
     * nvuai.removeCallback();
     *
     * // Remove all callbacks attached to end event:
     * nvuai.removeCallback('end');
     *
     * // Remove myFunction2 from being called on start:
     * nvuai.removeCallback('start', myFunction2);
     *
     * // Remove myFunction1 from being called on all events:
     * nvuai.removeCallback(undefined, myFunction1);
     * ````
     *
     ...
     * Returns false if speech recognition is off or nvuai is paused.
     *
     * @return boolean true = SpeechRecognition is on and nvuai is listening
     * @method isListening
     */
    ...
    /**
     * Returns the instance of the browser's SpeechRecognition object used by nvuai.
     * Useful in case you want direct access to the browser's Speech Recognition engine.
     *
     * @returns SpeechRecognition The browser's Speech Recognizer currently used by nvuai
     ...
     * #### Examples:
     * ````javascript
     * nvuai.trigger('Time for some thrilling heroics');
     * nvuai.trigger(
    ...
    trigger: function trigger(sentences) {
      if (!nvuai.isListening()) {
        if (debugState) {
          if (!_isListening) {
            logMessage('Cannot trigger while nvuai is aborted');
          } else {
            logMessage('Speech heard, but nvuai is paused');
          }
        }
        return;
      }
    ...
    /**
     * Initialize nvuai with a list of commands to recognize.
     *
     * #### Examples:
     * ````javascript
     ...
     * const commands2 = {'hi': helloFunction};
     *
     * // initialize nvuai, overwriting any previously added commands
     * nvuai.init(commands, true);
     * // adds an additional command without removing the previous commands
     * nvuai.init(commands2, false);
     * ````
     * As of v1.1.0 it is no longer required to call init(). Just start() listening whenever you want, and addCommands() whenever, and as often as you like.
     *
     * @param {Object} commands - Commands that nvuai should listen to
    ...
    init: function init(commands) {
      ...
      recognition.continuous = root.location.protocol === 'https:';

      // Sets the language to the default 'en-US'. This can be changed with nvuai.setLanguage()
      ...
      recognition.onend = function () {
        ...
        // nvuai will auto restart if it is closed automatically and not by user action.
        if (autoRestart) {
          // play nicely with the browser, and never restart nvuai automatically more than once per second
          var timeSinceLastStart = new Date().getTime() - lastStartedAt;
          autoRestartCount += 1;
          if (autoRestartCount % 10 === 0) {
            if (debugState) {
              logMessage('Speech Recognition is repeatedly stopping and starting. See http://is.gd/nvuai_restarts for tips.');
            }
          }
          if (timeSinceLastStart < 1000) {
            setTimeout(function () {
              nvuai.start({ paused: pauseListening });
 ...
 *
 * nvuai understands commands with `named variables`, `splats`, and `optional words`.
 *
 ...
 * <script>
 * const commands = {
 *   // nvuai will capture anything after a splat (*) and pass it to the function.
 ...
 *
 *   // By defining a part of the following command as optional, nvuai will respond
